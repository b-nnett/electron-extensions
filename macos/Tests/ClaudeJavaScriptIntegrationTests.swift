...
/// Opt-in native manager/helper proof for the existing Claude assisted new-chat profile.
/// Renderer observations come from a fixed imported test extension, never a
/// general inspection endpoint. A supervising user/agent performs the setup and two UI checkpoints.
@MainActor
final class ClaudeJavaScriptIntegrationTests: XCTestCase {
    private let appKey = "com.anthropic.claudefordesktop"
    private let helperID = "dev.extensionsanywhere.launcher.claude"
    ...
    func testOptInNativeClaudeAssistedImportedPackageLifecycle() async throws {
        guard ProcessInfo.processInfo.environment["EA_RUN_NATIVE_CLAUDE_PACKAGE_E2E"] == "1" else {
            throw XCTSkip("Set EA_RUN_NATIVE_CLAUDE_PACKAGE_E2E=1 for supervised Claude assisted new-chat proof with three native UI checkpoints.")
        }
        guard NSRunningApplication.runningApplications(withBundleIdentifier: appKey).isEmpty,
            ...
            throw failure("Claude or its launcher is already running. This test will not quit any pre-existing app.")
        }
        let app = try XCTUnwrap(ElectronAppDiscovery.scan(in: URL(fileURLWithPath: "/Applications"))
            .first { $0.url.path == "/Applications/Claude.app" && $0.bundleIdentifier == appKey })
        guard ElectronLaunchProfile.profile(for: app)?.supportsJavaScript == true else { throw failure("The exact Claude assisted new-chat profile is not JS-capable in this test adapter.") }
        ...
        let runID = UUID()
        let proof = repository.appendingPathComponent("output/release-review/2026-09-13/native-claude-package-\(runID.uuidString)")
        ...
        report = ["schema": 1, "passed": false, "startedAt": timestamp(), "managerBundle": packagedManager.path,
            ...
            "scope": "Exact Claude new-chat page only. Native manager/importer and generated helper with packaged broker; isolated library and memory Dock. Three supervised native UI checkpoints. Fixed imported observer reads only owned proof DOM/CSS. Official user-enabled debugger setup is required after each launch. No other chat routes, generic inspector evaluator, or manager-GUI-import claim."]
        let dock = MemoryDock()
        ...
        let helper = launcher.directory(for: app).appendingPathComponent("Claude Launcher.app")
        let subjectFolder = live.appendingPathComponent("Subject")
        let observerFolder = live.appendingPathComponent("Observer")
        let proofClass = "ea-claude-native-proof-\(runID.uuidString.lowercased())"
        ...
        do {
            ...
            report["managerExecutableSHA256"] = try fileHash(packagedManager.appendingPathComponent("Contents/MacOS/ExtensionsAnywhere"))
            report["compiledTestAdapterSHA256"] = try fileHash(try XCTUnwrap(Bundle(for: Self.self).executableURL))
            report["testAdapterSourceSHA256"] = try fileHash(URL(fileURLWithPath: #filePath))
            report["compiledAdapterIsManagerGUI"] = false
            ...
            report["helperExecutableSHA256"] = try fileHash(try XCTUnwrap(Bundle(url: helper)?.executableURL))
            ...
            try require(configuration["libraryPath"] as? String == manager.storageFileURL.path &&
                ...
                configuration["brokerPath"] as? String == helper.appendingPathComponent("Contents/Resources/Runtime/scripts/dock-claude-session.mjs").path,
            ...
            report["observerSourceSHA256"] = try fileHash(observerFolder.appendingPathComponent("observer.js"))
            manager.open(app)
            try require(manager.errorMessage == nil && dock.opened == [launcher.shortcut(for: app)], "Open App dispatches the generated alias")
            let directory: URL = try await wait("Claude assisted session starts", timeout: 60) {
                ...
                session = directory; self.report["session"] = directory.path
                if value["phase"] as? String == "error" { throw self.failure(value["error"] as? String ?? "Claude broker failed") }
                return ["waiting-for-debugger", "waiting-for-page", "active"].contains(value["phase"] as? String ?? "") ? directory : nil
            }
            session = directory; report["session"] = directory.path
            let setupReport = try json(directory.appendingPathComponent("report.json"), limit: 1024 * 1024)
            try require(setupReport["launchOwnership"] as? String == "normal-launch-requested",
                "The broker normally launched Claude for this test rather than borrowing a pre-existing instance")
            let setupStatus = try json(directory.appendingPathComponent("status.json"))
            report["setupStatus"] = setupStatus
            if setupStatus["phase"] as? String != "active" {
                let setupSnapshot = try XCTUnwrap(launcher.runtimeSession(app: app))
                try require(setupSnapshot.isWaitingForAssistedSetup && !setupSnapshot.isHealthy,
                    "Native status shows assisted setup without claiming running extensions")
                report["setupInstruction"] = setupSnapshot.assistedSetupStatus
                try checkpoint(0, action: "In Claude choose Developer → Enable Main Process Debugger and open a new chat (https://claude.ai/new). If the Developer menu is absent, enable Developer Mode through Help → Troubleshooting first; a Claude restart ends this run and requires a fresh proof. Do not enter a message or change private preferences.", proof: proof)
                let _: Bool = try await wait("official Claude debugger setup and observer connection", timeout: 180) {
                    let value = try self.json(directory.appendingPathComponent("status.json"))
                    if ["error", "stopped"].contains(value["phase"] as? String ?? "") {
                        throw self.failure(value["error"] as? String ?? "Claude ended during assisted setup; run a fresh proof after normal setup")
                    }
                    return value["phase"] as? String == "active" ? true : nil
                }
            } else {
                report["setupAlreadyActive"] = true
            }
            try require(launcher.runtimeSession(app: app)?.isHealthy == true, "Native status identifies the live Claude/helper session")
            ...
            try require(manager.errorMessage == nil, "Native enable persists mixed CSS and JS for the assisted new-chat profile")
            ...
            let initial = try await observation(observer: observer, session: directory) {
                ...
                self.isVisibleProofHitTarget($0)
            }
            report["initialObserverSnapshot"] = initial
            ...
            try checkpoint(1, action: "Click the green ‘Claude extension proof’ button once in Claude. Do not send a message or click an account control.", proof: proof)
            _ = try await observation(observer: observer, session: directory, timeout: 180) {
                $0["document"] as? String == initialDocument && $0["extensionID"] as? String == subject.id.uuidString &&
                $0["clicks"] as? Int == 1 && self.isVisibleProofHitTarget($0)
            }
            let clicked = try await subjectLog(subject, session: directory, prefix: "EA_CLAUDE_CLICK ")
            ...
            _ = try await observation(observer: observer, session: directory) {
                $0["count"] as? Int == 1 && $0["extensionID"] as? String == subject.id.uuidString &&
                $0["clicks"] as? Int == 0 && $0["outline"] as? String == "3px" && self.isVisibleProofHitTarget($0)
            }
            ...
            report["replacementSourceHashes"] = try sourceHashes(subjectFolder)
            ...
            let replaced = try await observation(observer: observer, session: directory) {
                $0["count"] as? Int == 1 && $0["extensionID"] as? String == subject.id.uuidString &&
                $0["outline"] as? String == "7px" && $0["clicks"] as? Int == 0 && self.isVisibleProofHitTarget($0)
            }
            let ready = try await subjectLog(subject, session: directory, prefix: "EA_CLAUDE_READY ")
            ...
            try checkpoint(2, action: "Use Claude’s native Reload command, wait for the green ‘Claude extension proof’ button to return, then click it once.", proof: proof)
            ...
            let postReload = try await subjectLog(subject, session: directory, prefix: "EA_CLAUDE_CLICK ")
            ...
            try require(disabledTarget.status == "ok" && disabledTarget.started == targetIdentity["started"] as? String &&
                disabledTarget.executable == app.url.appendingPathComponent("Contents/MacOS/Claude").path && disabledTarget.uid == getuid() &&
                disabledHelper.status == "ok" && disabledHelper.executable == Bundle(url: helper)?.executableURL?.path && disabledHelper.uid == getuid(),
                "Disabling the last extension preserves the exact running Claude instance and its assisted extension helper")
            ...
            try require(NSDictionary(dictionary: try XCTUnwrap(report["before"] as? [String: Any])).isEqual(to: after),
                "Claude signing and checked executable/archive/signature resources are unchanged")
            report["after"] = after
            report["passed"] = true
        } catch { operationError = error }
        // End only this test's generated helper. It must detach its bridge and
        // leave Claude alive; the supervisor may quit that app normally later.
        ...
        do {
            ...
            if let session {
                ...
                report["vendorVersion"] = broker["appVersion"]; report["vendorBuild"] = broker["appBuild"]
                try require(!(broker["appVersion"] as? String ?? "").isEmpty && !(broker["appBuild"] as? String ?? "").isEmpty,
                    "Broker records the installed Claude version and build")
                ...
                try require(cleanup["jsCleanupVerified"] as? Bool == true && cleanup["stylesheetRemoved"] as? Bool == true &&
                    cleanup["rendererDebuggerDetached"] as? Bool == true && cleanup["targetTerminationRequested"] as? Bool == false &&
                    cleanup["controllerDisconnected"] as? Bool == true,
                    "Explicit helper stop cleans scripts and styles and detaches only its owned renderer/main-inspector clients")
                let finalObservation = try XCTUnwrap(cleanup["finalProcessObservation"] as? [String: Any])
                try require(cleanup["launchedAppLeftRunning"] as? Bool == true && finalObservation["sameProcessAlive"] as? Bool == true,
                    "Broker reports the original Claude process still running after bridge cleanup")
                ...
                report["afterHelperTargetIdentity"] = try JSONSerialization.jsonObject(with: JSONEncoder().encode(fresh))
                try require(fresh.status == "ok" && fresh.started == originalIdentity["started"] as? String &&
                    fresh.executable == "/Applications/Claude.app/Contents/MacOS/Claude" && fresh.uid == getuid(),
                    "Independent kernel observation confirms the same Claude process survived helper exit")
                report["targetQuitByTest"] = false
        ...
        print("Native Claude assisted proof result: \(proof.appendingPathComponent("native-report.json").path)")
    ...
    private func observation(observer: ExtensionRecord, session: URL, timeout: TimeInterval = 30,
        ...
        try await wait("fixed observer DOM/CSS readback", timeout: timeout) {
            ...
            if status["phase"] as? String == "error" { throw self.failure(status["error"] as? String ?? "Claude extension failed") }
            // Only the newest snapshot is eligible; old matching history cannot
            // prove a later disable/re-enable/reload checkpoint.
            guard let last = try self.logs(observer, session: session, file: "observer.js").last(where: { $0.message.hasPrefix("EA_CLAUDE_OBSERVE ") }),
                  let data = String(last.message.dropFirst("EA_CLAUDE_OBSERVE ".count)).data(using: .utf8),
                  let value = try JSONSerialization.jsonObject(with: data) as? [String: Any] else { return nil }
            self.report["lastObserverSnapshot"] = value
            self.report["lastObserverTimestamp"] = ISO8601DateFormatter().string(from: last.timestamp)
            return matches(value) ? value : nil
        }
    }
    private func isVisibleProofHitTarget(_ snapshot: [String: Any]) -> Bool {
        snapshot["checkVisibility"] as? Bool == true && snapshot["centerInViewport"] as? Bool == true &&
        snapshot["centerHitsOwnedButton"] as? Bool == true && snapshot["documentHidden"] as? Bool == false
    }
    ...
    private func checkpoint(_ number: Int, action: String, proof: URL) throws {
        let value: [String: Any] = ["schema": 1, "checkpoint": number, "app": "/Applications/Claude.app", "action": action, "timeoutSeconds": 180, "createdAt": timestamp()]
        ...
        print("CLAUDE CHECKPOINT \(number): \(action) Evidence: \(proof.path)")
    }

    private func writeSubject(in folder: URL, proofClass: String, replacement: Bool) throws {
        try FileManager.default.createDirectory(at: folder, withIntermediateDirectories: true, attributes: [.posixPermissions: 0o700])
        let manifest = ExtensionManifest(name: "Claude new-chat proof", description: "Temporary authored button with registered cleanup", version: replacement ? "1.0.1" : "1.0.0", css: ["styles.css"], js: ["main.js"])
        ...
        let js = """
        if (location.href !== 'https://claude.ai/new') throw new Error('Expected Claude new-chat page');
        const className = '\(proofClass)';
        if (document.querySelector('.' + className)) throw new Error('Duplicate proof button');
        const button = document.createElement('button');
        button.id = 'ea-claude-proof-' + ea.id; button.type = 'button'; button.className = className;
        button.dataset.extensionId = ea.id; button.dataset.clickCount = '0'; button.textContent = 'Claude extension proof';
        let count = 0;
        const click = () => { count += 1; button.dataset.clickCount = String(count); button.textContent = 'Claude extension proof (' + count + ')'; console.log('EA_CLAUDE_CLICK ' + JSON.stringify({ count, revision: '\(replacement ? "replacement" : "initial")' })); };
        ea.onDispose(() => { button.removeEventListener('click', click); button.remove(); console.info('EA_CLAUDE_REMOVED ' + ea.id); });
        button.addEventListener('click', click, { signal: ea.signal }); document.body.append(button);
        console.info('EA_CLAUDE_READY \(replacement ? "replacement" : "initial")');
        """
    ...
    private func writeObserver(in folder: URL, proofClass: String) throws {
        try FileManager.default.createDirectory(at: folder, withIntermediateDirectories: true, attributes: [.posixPermissions: 0o700])
        let manifest = ExtensionManifest(name: "Claude proof observer", description: "Observes only owned proof DOM and CSS markers", version: "1.0.0", css: [], js: ["observer.js"])
        try JSONEncoder().encode(manifest).write(to: folder.appendingPathComponent("manifest.json"), options: .atomic)
        let js = """
        if (location.href !== 'https://claude.ai/new') throw new Error('Expected Claude new-chat page');
        ...
        const observe = () => {
          ...
          const rect = bounds ? { x: bounds.x, y: bounds.y, width: bounds.width, height: bounds.height } : null;
          ...
          const center = bounds ? { x: bounds.x + bounds.width / 2, y: bounds.y + bounds.height / 2 } : null;
          const centerInViewport = Boolean(bounds && bounds.width > 0 && bounds.height > 0 && center.x >= 0 && center.y >= 0 && center.x < innerWidth && center.y < innerHeight);
          // Read only whether the centre hits our own button; never record the
          // other element's identity, text, attributes or surrounding app content.
          const hit = centerInViewport ? document.elementFromPoint(center.x, center.y) : null;
          const centerHit = hit === button && button ? 'self' : hit && button?.contains(hit) ? 'descendant' : hit ? 'other' : 'none';
          const checkVisibility = button && typeof button.checkVisibility === 'function' ? button.checkVisibility({ opacityProperty: true, visibilityProperty: true, contentVisibilityAuto: true }) : null;
          const value = JSON.stringify({ document: documentToken, count: buttons.length, extensionID: button?.dataset.extensionId ?? '', clicks: Number(button?.dataset.clickCount ?? 0), background: button ? getComputedStyle(button).backgroundColor : '', outline: getComputedStyle(document.documentElement).outlineOffset,
            rect, viewport, center, checkVisibility, centerInViewport, centerHit, centerHitsOwnedButton: centerHit === 'self' || centerHit === 'descendant', documentHidden: document.hidden });
          if (value !== previous) { previous = value; console.info('EA_CLAUDE_OBSERVE ' + value); }
        };
        const timer = setInterval(observe, 250);
        ea.onDispose(() => { clearInterval(timer); console.info('EA_CLAUDE_OBSERVER_REMOVED'); });
        observe();
        """
    ...
    private func integrity(_ app: URL, proof: URL) throws -> [String: Any] {
        guard app.path == "/Applications/Claude.app" else { throw failure("Unexpected integrity target") }
        ...
        for file in ["Contents/Info.plist", "Contents/MacOS/Claude", "Contents/Resources/app.asar", "Contents/_CodeSignature/CodeResources"] {
    ...
    private func sourceHashes(_ folder: URL) throws -> [String: String] {
        try Dictionary(uniqueKeysWithValues: ["manifest.json", "styles.css", "main.js"].map {
            ($0, try fileHash(folder.appendingPathComponent($0)))
        })
    }
    private func fileHash(_ file: URL) throws -> String {
        guard let data = try RuntimeBoundedFile.read(file, maximumBytes: 64 * 1024 * 1024) else { throw failure("Missing bounded proof artifact \(file.lastPathComponent)") }
    ...
    private func failure(_ message: String) -> NSError {
        NSError(domain: "ExtensionsAnywhere.NativeClaudeProof", code: 1, userInfo: [NSLocalizedDescriptionKey: message])
    }
}
